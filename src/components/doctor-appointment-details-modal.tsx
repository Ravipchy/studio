...
import { DoctorAppointment } from "./doctor-appointments-list";
import { Calendar, Clock, MapPin, Video, User, FileText, Phone, Mail } from "lucide-react";
import Image from "next/image";

interface DoctorAppointmentDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  appointment: DoctorAppointment | null;
}

export function DoctorAppointmentDetailsModal({ isOpen, onClose, appointment }: DoctorAppointmentDetailsModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Appointment with {appointment.patient.name}</DialogTitle>
          <DialogDescription>
        ...
        <div className="py-4 space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="flex items-center gap-4">
                <Image src={appointment.patient.avatar} alt={appointment.patient.name} width={80} height={80} className="rounded-full border" data-ai-hint="patient avatar" />
                <div>
                    <h3 className="font-bold text-lg">{appointment.patient.name}</h3>
                    <p className="text-muted-foreground">Age: {appointment.patient.age}</p>
            ...
            <div>
                ...
                <div className="space-y-3 text-sm">
                    ...
                    <div className="flex items-start gap-3">
                       {appointment.type === "Clinic" && <MapPin className="h-5 w-5 text-primary mt-0.5" />}
